...
class ElaborateEraModel era where
  ...
  elaborateTx proxy _ maxTTL (ModelTx mtxId mtxInputs mtxOutputs mtxFee mtxDCert mtxWdrl) = State.runState $ do
    ...
    wdrl <- fmap Map.fromList $
      for (Map.toList mtxWdrl) $ \(mAddr, qty) -> do
    ...
    let realTxBody = makeTxBody proxy maxTTL (Coin . unModelValue $ mtxFee) ins (StrictSeq.fromList outs) (StrictSeq.fromList dcerts) (Shelley.Wdrl wdrl)
