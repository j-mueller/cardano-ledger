...
Note that because of this potential slot length change nuance, Plutus scripts will
not receive any information in terms of slot numbers (no native scripts, no
validity intervals as slots, and not the current slot). Native timelock scripts
...
\textbf{Know your contract arguments.}
  ...
  \begin{itemize}
    \item Datum objects are required for all output-locking non-native scripts. That is, a
    ...
    \item Redeemers are required for all non-native scripts.
  \end{itemize}
...
\begin{figure}
  \begin{equation}
    ...
    \inference[UTxO-witG]
    {
      ...
      \hldiff{\{ h \mid (\_ \mapsto (a, \_, h)) \in \txins{txb} \restrictdom \var{utxo}, \fun{isTwoPhaseScriptAddress}~{tx}~{a}\} } \\
